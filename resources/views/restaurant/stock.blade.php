...
@section('title', 'Restaurant - Gestion du Stock')
@section('content')
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h3 class="mb-0 text-primary">{{ $ingredients->count() }}</h3>
                    </div>
...
@php $lowItems = $ingredients->filter(fn($i) => $i->isLowStock()); @endphp
@if($lowItems->count() > 0)
<div class="alert alert-warning border-0 shadow-sm mb-4" role="alert">
    <div class="d-flex align-items-center mb-2">
        <i class="fas fa-bell me-2"></i>
        <strong>{{ $lowItems->count() }} alerte(s) de stock</strong>
    </div>
    <div class="d-flex flex-wrap gap-2">
        @foreach($lowItems as $item)
        <span class="badge {{ $item->quantity_in_stock <= 0 ? 'bg-danger' : 'bg-warning text-dark' }} px-3 py-2">
            <i class="fas fa-{{ $item->quantity_in_stock <= 0 ? 'times' : 'exclamation' }} me-1"></i>
            {{ $item->name }} — {{ $item->quantity_in_stock }} {{ $item->unit }}
            {{ $item->quantity_in_stock <= 0 ? '(RUPTURE)' : '(min: '.$item->min_stock.')' }}
        </span>
        @endforeach
    </div>
</div>
            ...
            <table class="table table-hover align-middle mb-0" id="ingredientsTable">
                ...
                <tbody>
                    ...
                    <tr class="ingredient-row">
                        ...
                        <td>
                            <span class="fw-bold {{ $ingredient->quantity_in_stock <= 0 ? 'text-danger' : ($ingredient->isLowStock() ? 'text-warning' : 'text-success') }}">
                        ...
                        <td>
                            ...
                            {{ number_format($ingredient->price_per_unit, 0, ',', ' ') }} FCFA
                            @else
                        ...
                        <td class="text-end">
                            <button class="btn btn-sm btn-outline-primary me-1"
                                onclick="openRestockModal({{ $ingredient->id }}, '{{ addslashes($ingredient->name) }}', {{ $ingredient->quantity_in_stock }}, '{{ $ingredient->unit }}', {{ $ingredient->min_stock }}, {{ $ingredient->price_per_unit ?? 0 }})"
                                title="Modifier / Réapprovisionner">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger"
                                onclick="deleteIngredient({{ $ingredient->id }}, '{{ addslashes($ingredient->name) }}')"
                                title="Supprimer">
...
function submitAddIngredient() {
    ...
    fetch('{{ route("restaurant.stock.store") }}', {
        method: 'POST',
