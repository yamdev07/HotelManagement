...
<section class="hero-lux" id="accueil">
    ...
    <div class="hero-content">
        <div class="eyebrow" data-aos="fade-down" style="color:#fff;opacity:.85;">
            @if($hotel->address)<i class="fas fa-location-dot me-2"></i>{{ $hotel->address }}@else {{ __('vitrine.hero_welcome') }} @endif
        </div>
        ...
        <div data-aos="fade-up" data-aos-delay="400" class="d-flex flex-wrap gap-3 justify-content-center">
            @if ($hotel->show_rooms)<a href="{{ route('public.hotel.rooms', $hotel->slug) }}" class="btn-c">{{ __('vitrine.hero_discover') }}</a>@endif
            @if ($hotel->show_contact)<a href="{{ route('public.hotel.contact', $hotel->slug) }}" class="btn-ghost">{{ __('vitrine.hero_contact') }}</a>@endif
        </div>
    </div>
