...
@section('title', __('vitrine.restaurant_title'))
...
@section('content')
    ...
    <header class="page-head {{ $cover ? 'has-img' : '' }}" @if($cover) style="background-image:url('{{ $cover }}')" @endif>
        ...
        <div class="container">
            <div class="eyebrow mb-2" style="color:#fff;opacity:.85;">{{ __('vitrine.restaurant_eyebrow') }}</div>
            <h1 class="display-serif" style="font-size:clamp(2.4rem,6vw,4rem);">{{ __('vitrine.restaurant_heading') }}</h1>
        </div>
    </header>
    ...
    <section class="section">
        <div class="container">
            <div class="text-center mb-5" data-aos="fade-up">
                <p class="text-secondary" style="max-width:600px;margin-inline:auto;font-size:1.1rem;">{{ __('vitrine.restaurant_intro') }}</p>
            </div>

            @if ($menus->isEmpty())
                <p class="text-center text-secondary">{{ __('vitrine.restaurant_empty') }}</p>
            @else
