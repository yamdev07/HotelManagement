...
@section('title', __('public_restaurant.title'))
...
@section('content')
    ...
    <header class="page-head {{ $cover ? 'has-img' : '' }}" @if($cover) style="background-image:url('{{ $cover }}')" @endif>
        ...
        <div class="container">
            <div class="eyebrow mb-2" style="color:#fff;opacity:.85;">{{ __('public_restaurant.subtitle') }}</div>
            <h1 class="display-serif" style="font-size:clamp(2.4rem,6vw,4rem);">{{ __('public_restaurant.our_restaurant') }}</h1>
        </div>
    </header>
    ...
    <section class="section">
        <div class="container">
            <div class="text-center mb-5" data-aos="fade-up">
                <p class="text-secondary" style="max-width:600px;margin-inline:auto;font-size:1.1rem;">{{ __('public_restaurant.description') }}</p>
            </div>

            @if ($menus->isEmpty())
                <p class="text-center text-secondary">{{ __('public_restaurant.menu_coming') }}</p>
            @else
