...
@section('title', __('vitrine.contact_title'))
...
@section('content')
    ...
    <header class="page-head {{ $cover ? 'has-img' : '' }}" @if($cover) style="background-image:url('{{ $cover }}')" @endif>
        ...
        <div class="container">
            <div class="eyebrow mb-2" style="color:#fff;opacity:.85;">{{ __('vitrine.contact_eyebrow') }}</div>
            <h1 class="display-serif" style="font-size:clamp(2.4rem,6vw,4rem);">{{ __('vitrine.contact_heading') }}</h1>
        </div>
    </header>
                ...
                @if ($hotel->contact_phone)
                    <div class="col-md-4" data-aos="fade-up">
                        <div class="text-center p-5 lift" style="background:#faf9f7;border-radius:6px;">
                            ...
                            <h4 class="serif mb-2" style="font-size:1.3rem;">{{ __('vitrine.contact_phone') }}</h4>
                ...
                @if ($hotel->contact_email)
                    <div class="col-md-4" data-aos="fade-up" data-aos-delay="120">
                        <div class="text-center p-5 lift" style="background:#faf9f7;border-radius:6px;">
                            ...
                            <h4 class="serif mb-2" style="font-size:1.3rem;">{{ __('vitrine.contact_email') }}</h4>
                ...
                @if ($hotel->address)
                    <div class="col-md-4" data-aos="fade-up" data-aos-delay="240">
                        <div class="text-center p-5 lift" style="background:#faf9f7;border-radius:6px;">
                            ...
                            <h4 class="serif mb-2" style="font-size:1.3rem;">{{ __('vitrine.contact_address') }}</h4>
                            <p class="text-secondary mb-0">{{ $hotel->address }}</p>
                        </div>
                    </div>
                @endif
                @if (! $hotel->contact_phone && ! $hotel->contact_email && ! $hotel->address)
                    <div class="col-12 text-center text-secondary">{{ __('vitrine.contact_coming_soon') }}</div>
                @endif
            </div>
        </div>
    </section>
