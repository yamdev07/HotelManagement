...
@section('title', __('public_booking_confirmed.title'))
...
@section('content')
    ...
    <section class="section" style="padding-top:120px;">
        <div class="container cf-wrap">
            ...
            <h1 class="display-serif" style="font-size:clamp(1.9rem,5vw,2.7rem);">{{ __('public_booking_confirmed.recorded') }}</h1>
            <p style="color:#6b7280; max-width:440px; margin:8px auto 0;">
                {{ __('public_booking_confirmed.thanks_message', ['name' => $tx->customer->name ?? '', 'hotel' => $hotel->name]) }}
            </p>
            <div class="cf-ref">{{ $ref }}</div>

            <div class="cf-card">
                <div class="hd">{{ __('public_booking_confirmed.details') }}</div>
                <div class="cf-body">
                    <div class="cf-line"><span>{{ __('public_booking_confirmed.room') }}</span> <strong>{{ $tx->room->type->name ?? __('public_booking.room') }} · n°{{ $tx->room->number ?? '-' }}</strong></div>
                    <div class="cf-line"><span>{{ __('public_booking_confirmed.arrival') }}</span> <strong>{{ $ci->translatedFormat('D d M Y') }}</strong></div>
                    <div class="cf-line"><span>{{ __('public_booking_confirmed.departure') }}</span> <strong>{{ $co->translatedFormat('D d M Y') }}</strong></div>
                    <div class="cf-line"><span>{{ __('public_booking_confirmed.guests') }}</span> <strong>{{ $tx->person_count }}</strong></div>
                    <div class="cf-line cf-total"><span>{{ __('public_booking_confirmed.total_stay') }}</span> <strong>{{ number_format($tx->total_price, 0, ',', ' ') }} {{ $hotel->currency }}</strong></div>
                    <div class="cf-line"><span>{{ __('public_booking_confirmed.suggested_deposit') }}</span> <strong style="color:var(--c)">{{ number_format($deposit, 0, ',', ' ') }} {{ $hotel->currency }}</strong></div>
                </div>
            </div>

            @if (session('payment_success'))
                <div class="cf-flash ok"><i class="fas fa-circle-check"></i> {{ __('public_booking_confirmed.payment_received') }}</div>
            @endif
            ...
            @if ($depositPaid)
                ...
                <div class="cf-paid">
                    ...
                    {{ __('public_booking_confirmed.deposit_paid', ['amount' => number_format($deposit, 0, ',', ' '), 'currency' => $hotel->currency]) }}
                </div>
                <div class="cf-next">
                    <strong><i class="fas fa-circle-info"></i> {{ __('public_booking_confirmed.next_step') }}</strong><br>
                    {{ __('public_booking_confirmed.balance_message', ['amount' => number_format(max(0, $tx->total_price - $deposit), 0, ',', ' '), 'currency' => $hotel->currency]) }}
                </div>
            @elseif ($canPayOnline)
                {{-- Paiement en ligne de l'acompte --}}
                <div class="cf-pay">
                    <h3><i class="fas fa-lock" style="color:var(--c)"></i> {{ __('public_booking_confirmed.secure_title') }}</h3>
                    <p>{{ __('public_booking_confirmed.secure_description') }}</p>
                    ...
                    <form method="POST" action="{{ route('public.hotel.payment.pay', [$hotel->slug, $tx->id]) }}">
                        @csrf
                        <button type="submit" class="cf-paybtn"><i class="fas fa-credit-card"></i> {{ __('public_booking_confirmed.pay_deposit') }}</button>
                    </form>
                    <div class="cf-secure"><i class="fas fa-shield-halved"></i> {{ __('public_booking_confirmed.secure_text') }}</div>
                </div>
                <div class="cf-next">
                    {{ __('public_booking_confirmed.balance_message', ['amount' => number_format(max(0, $tx->total_price - $deposit), 0, ',', ' '), 'currency' => $hotel->currency]) }}
                </div>
            @else
                ...
                <div class="cf-next">
                    <strong><i class="fas fa-circle-info"></i> {{ __('public_booking_confirmed.next_step') }}</strong><br>
                    {{ __('public_booking_confirmed.hotel_contact', ['deposit' => number_format($deposit, 0, ',', ' '), 'currency' => $hotel->currency]) }}
                </div>
            @endif
                ...
                <a href="{{ route('public.checkin.show', $tx->checkinToken()) }}"
                   style="display:flex;align-items:center;gap:14px;margin-top:22px;padding:16px 18px;border-radius:14px;text-decoration:none;background:color-mix(in srgb, var(--c) 10%, #fff);border:1px solid color-mix(in srgb, var(--c) 30%, #fff);">
                    ...
                    <span style="flex:1;text-align:left;">
                        <strong style="display:block;color:#1f2733;">{{ __('public_booking_confirmed.preregister_title') }}</strong>
                        <small style="color:#6b7280;">{{ __('public_booking_confirmed.preregister_subtitle') }}</small>
                    </span>
                ...
                @if ($wa)
                    <a class="cf-btn wa" target="_blank" rel="noopener"
                       href="https://wa.me/{{ $wa }}?text={{ urlencode("Bonjour {$hotel->name}, je confirme ma réservation {$ref} (chambre ".($tx->room->number ?? '')." du ".$ci->format('d/m/Y')." au ".$co->format('d/m/Y').").") }}">
                        <i class="fab fa-whatsapp"></i> {{ __('public_booking_confirmed.contact_whatsapp') }}
                    </a>
                @endif
                <a class="cf-btn ghost" href="{{ route('public.hotel', $hotel->slug) }}"><i class="fas fa-house"></i> {{ __('public_booking_confirmed.back_to_site') }}</a>
            </div>
        </div>
    </section>
@endsection
