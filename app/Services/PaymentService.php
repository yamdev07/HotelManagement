...
class PaymentService
{
    ...
    private function buildDescription(array $data): string
    {
        ...
        switch ($data['payment_method']) {
            case 'mobile_money':
                if (! empty($data['mobile_operator'])) {
                    $details[] = "Opérateur: {$data['mobile_operator']}";
                }
                if (! empty($data['mobile_number'])) {
                    $details[] = "Tél: {$data['mobile_number']}";
                }
                if (! empty($data['transaction_id'])) {
                    $details[] = "ID: {$data['transaction_id']}";
                }
                break;

            case 'card':
                if (! empty($data['card_number'])) {
                    $details[] = 'Carte: **** '.substr($data['card_number'], -4);
                }
                if (! empty($data['card_type'])) {
                    $details[] = "Type: {$data['card_type']}";
                }
                if (! empty($data['card_holder'])) {
                    $details[] = "Titulaire: {$data['card_holder']}";
                }
                break;

            case 'transfer':
                if (! empty($data['bank_name'])) {
                    $details[] = "Banque: {$data['bank_name']}";
                }
                if (! empty($data['iban'])) {
                    $details[] = "IBAN: {$data['iban']}";
                }
                if (! empty($data['transfer_date'])) {
                    $details[] = 'Date: '.Carbon::parse($data['transfer_date'])->format('d/m/Y');
                }
                break;

            case 'fedapay':
                if (! empty($data['fedapay_transaction_id'])) {
                    $details[] = "Réf FedaPay: {$data['fedapay_transaction_id']}";
                }
                if (! empty($data['fedapay_method'])) {
                    $details[] = "Via: {$data['fedapay_method']}";
                }
                break;

            case 'check':
                if (! empty($data['check_number'])) {
                    $details[] = "Chèque n°{$data['check_number']}";
                }
                if (! empty($data['issuing_bank'])) {
                    $details[] = "Banque: {$data['issuing_bank']}";
                }
                break;
        }
